...
begin
  ...
  if v_uid is null
     ...
     and nullif(trim(coalesce(p_payload->>'_auth_uid', '')), '') is not null then
    begin
      v_uid := nullif(trim(p_payload->>'_auth_uid'), '')::uuid;
...
GRANT EXECUTE ON FUNCTION public.place_order(jsonb) TO service_role, postgres;

COMMENT ON FUNCTION public.place_order(jsonb) IS
  'Checkout: PKR delivery; email optional (guest + signed-in); phone + address required; service_role may pass _auth_uid.';
